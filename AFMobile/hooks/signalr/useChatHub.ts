// useChatHub.ts - Fikset versjon med riktig timing
    ...
    const startConnection = async () => {
      try {
        ...
        if (!conn) {
          ...
          conn = await createChatConnection();
          
          // Sett opp event listeners FØR vi starter tilkoblingen
          setupEventListeners(conn);
          
          console.log("🔧 Event listeners set up, attempting to start connection...");
        }
        ...
        if (conn.state === signalR.HubConnectionState.Disconnected) {
          ...
          console.log(`ℹ️ Connection already in state: ${conn.state}`);
        }
      } catch (err) {
