...
export default function FriendList() {
  const { friends, loading, loadMore, hasMore, loadingMore, removeFriend } = useFriends();
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredFriends, setFilteredFriends] = useState(friends);
  const { confirmAndRemove, ConfirmDialog } = useConfirmRemoveFriend(); // ✅ Destruktur ConfirmDialog
  ...
  return (
    <>
                  ...
                  actions={[
                    {
                      ...
                      onClick: () =>
                        confirmAndRemove(friend.friend.id, friend.friend.fullName, () => {
                          // Bruk removeFriend fra hook-en i stedet for lokal state
      ...
      {/* ✅ Viktig: Render ConfirmDialog komponenten */}
      <ConfirmDialog />
    </>
  );
}