...
export function MessageSettingsModalNative({
  ...
  const currentConversation = useConversationStore((s) =>
    ...
    (s.pendingConversations ?? []).find((c) => c.id === currentConversationId)
  );
  const participants = currentConversation?.participants || [];
  const isGroup = currentConversation ? isGroupConversation(currentConversation) : false;
  ...
  const handleDeleteConversation = async () => {
    ...
    const otherParticipant = currentConversation
      ? getOtherParticipant(currentConversation, currentUserId)
      : undefined;
    const conversationName = otherParticipant?.user.fullName || "this conversation";
    const isPending = currentConversation ? isPendingConversation(currentConversation) : false;
