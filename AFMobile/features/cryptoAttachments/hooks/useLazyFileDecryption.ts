...
// Helper function for consistent cache keys
const generateCacheKey = (url: string): string => {
  const parts = url.split('/');
  const filename = parts[parts.length - 1];
  
  // Remove .enc and extract base name
  // Example: "thumb_1000013195_8c0400ee.enc" -> "thumb_1000013195"
  const baseName = filename.replace('.enc', '');
  const nameParts = baseName.split('_');
  
  // Take the first two parts (thumb_filenumber)
  if (nameParts.length >= 2) {
    return `${nameParts[0]}_${nameParts[1]}`;
  }
  
  // Fallback to full name without .enc
  return baseName;
};
