...
export default function ProfileAvatarNative({
  ...
  const handleFilesSelected = useCallback(async (files: RNFile[]) => {
    ...
    try {
      ...
      showNotificationToastNative({
        ...
        customTitle: t("profile.pictureUpdatedTitle"),
        customBody: t("profile.pictureSelectError"),
        position: 'top'
      });
    }
  }, []);
  ...
  const handleUpload = async () => {
    if (!selectedFile || !userId) {
      showNotificationToastNative({
        type: LocalToastType.CustomSystemNotice,
        customTitle: t("profile.pictureUpdatedTitle"),
        customBody: t("profile.pictureUploadError"),
        position: 'top'
      });
      return;
    }
    ...
    try {
      ...
      if (uploadedUrl) {
        ...
        showNotificationToastNative({
          type: LocalToastType.CustomSystemNotice,
          customTitle: t("profile.pictureUpdatedTitle"),
          customBody: t("profile.pictureUpdatedBody"),
          position: 'top'
        });
      }
    } catch (err) {
  ...
  const handleRemoveImage = async () => {
    const confirmed = await confirm({
      ...
      message: t("profile.removePictureMessage"),
    });
    ...
    try {
      ...
      showNotificationToastNative({
        type: LocalToastType.CustomSystemNotice,
        customTitle: t("profile.pictureRemovedTitle"),
        customBody: t("profile.pictureRemovedBody"),
        position: 'top'
      });
    } catch (err) {
      ...
      showNotificationToastNative({
        type: LocalToastType.CustomSystemNotice,
        customTitle: t("profile.pictureRemovedTitle"),
        customBody: t("profile.pictureRemoveError"),
        position: 'top'
      });
    } finally {
      setIsRemoving(false);
          ...
          {!isEditMode ? (
            ...
            <View style={styles.defaultActionsContainer}>
              <AttachmentPicker
                ...
                imageQuality={0.8}
                cameraQuality={0.8}
                modalTitle={t("profile.selectPictureTitle")}
                useNativeButton={true}
                buttonText={t("profile.selectPictureTitle")}
                nativeButtonProps={{
                  variant: "primary",
                  size: "medium",
                  style: styles.editButton,
            ...
            <View style={styles.actionButtonsContainer}>
              ...
              <AttachmentPicker
                ...
                modalTitle={t("profile.selectPictureTitle")}
                useNativeButton={true}
                buttonText={t("profile.changePicture")}
                nativeButtonProps={{
                  variant: "secondary",
                  size: "medium",
