...
public class MessagesController : BaseController
{
    ...
    // Avslår meldingsforespørsel og putter avsender sin bruker i blokkeringsliste
    [HttpPost("decline-request")]
    public async Task<IActionResult> DeclineMessageRequest([FromBody] int senderId)
    {
        var receiverId = GetUserId();
        if (receiverId == null)
            return Unauthorized();
        ...
        try
        {
            await _messageService.DeclineMessageRequestAsync(receiverId.Value, senderId);
            return Ok(new { message = "Forespørsel avslått og avsender blokkert." });
        }
        catch (Exception ex)
        {
            return BadRequest(new { message = ex.Message });
