...
**Presence-basert IsRead:** Notifications opprettes med `IsRead = true` hvis brukeren er aktiv i samtalen (via ConversationPresenceService). Dette sikrer at brukere som ser hendelsen i sanntid ikke får unødvendige badges/toasts, samtidig som historikken bevares for sikkerhetsformål.
...
| `IsRead` | bool | Om varselet er lest (settes automatisk til `true` hvis bruker er aktiv i samtalen ved opprettelse) |
...
SendMessageAsync (MessageBroadcastService)
    │
    ├─► ConversationPresenceService.GetActiveUsersInConversationAsync()
    │       • Hent liste over aktive brukere i samtalen
    │
    ├─► For hver mottaker:
    │       │
    │       ├─► HVIS bruker er aktiv i samtalen:
    │       │       • Ingen notification opprettes
    │       │       • SyncEvent sendes med Notification = null
    │       │
    │       └─► HVIS bruker IKKE er aktiv:
    │               │
    │               └─► CreateNewMessageNotificationAsync (MessageNotificationService)
    │                       │
    │                       ├─► Sjekk om ulest notification eksisterer
    │                       │
    │                       ├─► HVIS eksisterer:
    │                       │       • MessageCount++
    │                       │       • Oppdater Summary
    ...
    │                       │
    │                       └─► HVIS ny:
    │                               • Opprett MessageNotification med IsRead = false
    │
    └─► Return SyncEvent med eller uten Notification
```
...
Service-metode (f.eks. AcceptPendingGroupConversationRequestAsync)
    ...
    └─► BroadcastService.BroadcastGroupXxxAsync
            │
            ├─► ConversationPresenceService.GetActiveUsersInConversationAsync()
            │       • Hent liste over aktive brukere
            │
            ├─► Bygg memberReadStatus dictionary:
            │       • { odId: true/false } basert på om bruker er aktiv
            │
            └─► GroupNotificationService.CreateGroupNotificationEventAsync(memberReadStatus)
                    │
                    ├─► For hver mottaker:
                    │       │
                    │       ├─► Hent isRead fra memberReadStatus
                    │       │
                    │       ├─► HVIS aktiv (isRead = true):
                    │       │       • Hent siste notification (uansett lest-status)
                    │       │       • Legg til event på eksisterende
                    │       │
                    │       ├─► HVIS ikke aktiv (isRead = false):
                    │       │       • Hent kun ulest notification
                    ...
                    │       │
                    │       └─► Hent alle GroupEvents for response
                    │
...
- **Presence-sjekk:** Hvis bruker er aktiv i samtalen, opprettes ingen notification

### GroupEvent (gruppe)
- Stacker alle hendelser i samme gruppe til én notification
...
- **Presence-sjekk:** Hvis bruker er aktiv:
  - Henter siste notification (uansett lest-status)
  - Legger til event på eksisterende notification
  - `IsRead` forblir som den var (lest notification forblir lest)
